...
class Create extends Command
{
    ...
    private function createController ($table)
    {
        ...
        $f .= "$fs"
            ...
            . "            if (isset(\$params[\$key])) {\n"
            . "                \$prcCurrentPriceDao->andEqual(\$params[\$key], \$value);\n"
            . "            }\n"
            . "            if (strtoupper(\$key) === 'ORDERBY') {\n"
            . "                foreach (\$value as \$v) {\n"
            . "                    \$parts = explode(',', \$v);\n"
            . "\n"
            . "                    if (isset(\$sort[\$parts[0]])) {\n"
            . "                        \$prcCurrentPriceDao->orderBy(\$sort[\$parts[0]], isset(\$parts[1]) ? \$parts[1] : 'ASC');\n"
            . "                    }\n"
            . "                }\n"
            . "            }\n"
            . "        }\n"
            ...
            foreach ($table->getColumns() as $column) {
                ...
                if (!in_array($column->getName(), ['ins_time', 'ins_user_id']) && !$column->getIsPrimary()) {
                    if (in_array($column->getName(), ['upd_time', 'upd_user_id'])) {
                        ...
                        $f .= "        \${$objectName}Dto->set{$s}(\$json->{$value});\n"
                            . "\n";
                    } else {
                        $f .= "        if (isset(\$json->{$c})) {\n"
                            . "            \${$objectName}Dto->set{$s}(\$json->{$c});\n"
                            . "        }\n"
                            . "\n";
                    }
                }
